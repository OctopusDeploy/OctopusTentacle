...
public class KubernetesScriptServiceV1IntegrationTest : KubernetesAgentIntegrationTest
{
    ...
    [Test]
    public async Task PodEventsAreWrittenToTaskLog()
    {
        // Arrange
        var logs = new List<ProcessOutput>();
        var scriptCompleted = false;
        ...
        var builder = new ExecuteKubernetesScriptCommandBuilder(LoggingUtils.CurrentTestHash())
            .WithScriptBody(script => script
                .Print("Hello World")
                .PrintNTimesWithDelay("Yep", 30, TimeSpan.FromMilliseconds(100)))
            //we run inside the bash container (as only this test will use it)
            .WithKubernetesImageConfiguration(new KubernetesImageConfiguration("bash",null, null,null));
        ...
        //Assert
        logs.Should().Contain(po => po.Source == ProcessOutputSource.Debug && po.Text.StartsWith("[POD EVENT] Scheduled |"));
        logs.Should().Contain(po => po.Source == ProcessOutputSource.Debug && po.Text.StartsWith("[POD EVENT] Created |"));
        logs.Should().Contain(po => po.Source == ProcessOutputSource.Debug && po.Text.StartsWith("[POD EVENT] Started |"));
        logs.Should().Contain(po => po.Source == ProcessOutputSource.StdOut && po.Text.StartsWith("[POD EVENT] Pulling |"));
        logs.Should().Contain(po => po.Source == ProcessOutputSource.StdOut && po.Text.StartsWith("[POD EVENT] Pulled | Successfully pulled image"));
        logs.Should().Contain(po => po.Source == ProcessOutputSource.StdOut && po.Text == "Hello World");
        scriptCompleted.Should().BeTrue();
        result.ExitCode.Should().Be(0);
        result.State.Should().Be(ProcessState.Complete);
        ...
        //act
        //now run the script again and we _shouldn't_ get the Pulling output
        result = await TentacleClient.ExecuteScript(command, StatusReceived, ScriptCompleted, new InMemoryLog(), CancellationToken);

        //Assert
        logs.Should().Contain(po => po.Source == ProcessOutputSource.Debug && po.Text.StartsWith("[POD EVENT] Scheduled |"));
        logs.Should().Contain(po => po.Source == ProcessOutputSource.Debug && po.Text.StartsWith("[POD EVENT] Created |"));
        logs.Should().Contain(po => po.Source == ProcessOutputSource.Debug && po.Text.StartsWith("[POD EVENT] Started |"));
        logs.Should().NotContain(po => po.Source == ProcessOutputSource.StdOut && po.Text.StartsWith("[POD EVENT] Pulling |"));
        logs.Should().Contain(po => po.Source == ProcessOutputSource.StdOut && po.Text.StartsWith("[POD EVENT] Pulled |") && po.Text.Contains("already present on machine"));
        logs.Should().Contain(po => po.Source == ProcessOutputSource.StdOut && po.Text == "Hello World");
        scriptCompleted.Should().BeTrue();
        result.ExitCode.Should().Be(0);
        result.State.Should().Be(ProcessState.Complete);

        recordedMethodUsages.For(nameof(IAsyncClientKubernetesScriptServiceV1.StartScriptAsync)).Started.Should().Be(2);
        recordedMethodUsages.For(nameof(IAsyncClientKubernetesScriptServiceV1.GetStatusAsync)).Started.Should().BeGreaterThan(2);
        recordedMethodUsages.For(nameof(IAsyncClientKubernetesScriptServiceV1.CompleteScriptAsync)).Started.Should().Be(2);
        recordedMethodUsages.For(nameof(IAsyncClientKubernetesScriptServiceV1.CancelScriptAsync)).Started.Should().Be(0);
