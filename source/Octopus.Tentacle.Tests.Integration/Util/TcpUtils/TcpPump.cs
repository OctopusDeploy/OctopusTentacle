            ...
            Task.Run(
                async () =>
                {
                    ...
                    try
                    {
                        ...
                        var pump1 = Task.Run(async () => await PumpBytes(clientSocket, originSocket, new SocketPump(() => this.IsPaused, sendDelay, biDirectionalCallBack.DataTransferObserverToRemote), cancellationToken).ConfigureAwait(false), cancellationToken);
                        var pump2 = Task.Run(async () => await PumpBytes(originSocket, clientSocket, new SocketPump(() => this.IsPaused, sendDelay, biDirectionalCallBack.DataTransferObserverFromRemote), cancellationToken).ConfigureAwait(false), cancellationToken);
