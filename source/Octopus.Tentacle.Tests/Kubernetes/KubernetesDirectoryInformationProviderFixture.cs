...
namespace Octopus.Tentacle.Tests.Kubernetes
{
    public class KubernetesDirectoryInformationProviderFixture
    {
        ...
        public void DuOutputParses()
        {
            ...
            var sut = new KubernetesDirectoryInformationProvider(Substitute.For<IKubernetesConfiguration>(), Substitute.For<ISystemLog>(), spr, memoryCache);
        ...
        public void IfDuFailsWeLogCorrectly()
        {
            ...
            spr.ReturnsForAll(1);
            var memoryCache = new MemoryCache(new MemoryCacheOptions());
            var sut = new KubernetesDirectoryInformationProvider(Substitute.For<IKubernetesConfiguration>(), systemLog, spr, memoryCache);
            sut.GetPathUsedBytes("/octopus").Should().Be(usedSize);
