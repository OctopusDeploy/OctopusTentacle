...
namespace Octopus.Tentacle.Configuration.Proxy
{
    ...
    {
        private static readonly Regex UrlEncodedCharactersRegex = new(@"%[A-F0-9]{2}", RegexOptions.Compiled);
        ...
        public IEnumerable<string> GetProxyPasswordMaskValues(string? proxyPassword)
        {
            ...
            var upperCaseUrlEncodedProxyPassword = WebUtility.UrlEncode(proxyPassword);
            var lowerCaseUrlEncodedProxyPassword = UrlEncodedCharactersRegex.Replace(upperCaseUrlEncodedProxyPassword, m => m.Value.ToLowerInvariant());
