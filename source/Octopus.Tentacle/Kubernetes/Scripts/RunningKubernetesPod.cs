...
namespace Octopus.Tentacle.Kubernetes.Scripts
{
    public class RunningKubernetesPod : IRunningScript
    {
        ...
        async Task<string?> CreateImagePullSecret(CancellationToken cancellationToken)
        {
            ...
            if (command.FeedUrl is null || command.FeedUsername is null)
                return null;

            var secretName = CreateImagePullSecretName(command.FeedUrl, command.FeedUsername);
            ...
            var config = new Dictionary<string, object>
            {
                ["auths"] = new Dictionary<string, object>
                {
                    [command.FeedUrl] = new
                    {
                        username = command.FeedUsername,
                        password = command.FeedPassword,
                        auth = Convert.ToBase64String(Encoding.UTF8.GetBytes($"{command.FeedUsername}:{command.FeedPassword}"))
                    }
                }
            };
            ...
            var pod = new V1Pod
            {
                ...
                Spec = new V1PodSpec
                {
                    Containers = new List<V1Container>
                    {
                        ...
                        {
                            Name = podName,
                            Image = command.Image ?? await containerResolver.GetContainerImageForCluster(),
