...
namespace Octopus.Tentacle.Commands
{
    ...
    public class SpaceRepositoryFactory : ISpaceRepositoryFactory
    {
        public async Task<IOctopusSpaceAsyncRepository> CreateSpaceRepository(IOctopusAsyncClient client, [CanBeNull] string spaceName)
        {
            ...
            {
                if (!await SupportsSpaces(client)) throw new ControlledFailureException($"A space with name \"{spaceName}\" was requested, but this version of Octopus Server does not support spaces. Please upgrade Octopus Server to a version which supports spaces, or remove the \"space\" parameter.");
                ...
                if (space == null) throw new ControlledFailureException($"A space with name \"{spaceName}\" could not be found. Ensure you have spelled the space name correctly and that the user has access to this space.");
            ...
            if (await SupportsSpaces(client))
            {
                ...
                if (defaultSpace == null) throw new ControlledFailureException("No \"space\" was specified, and the default space is disabled or inaccessible to this user. Please select a space using the \"space\" parameter.");
        ...
        private Task<bool> SupportsSpaces(IOctopusAsyncClient client)
        {
