        ...
        void SetupActive(OctopusServerConfiguration[] servers)
        {
            ...
            if (server.SubscriptionId == null)
            {
                var existingPollingConfiguration = servers.FirstOrDefault(s =>
                    (s.CommunicationStyle == CommunicationStyle.TentacleActive || (s.CommunicationStyle == CommunicationStyle.KubernetesAgent && s.AgentCommunicationBehaviour == AgentCommunicationBehaviour.Polling)) && s.SubscriptionId != null);
